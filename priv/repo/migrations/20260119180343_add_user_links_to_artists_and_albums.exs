defmodule Tunez.Repo.Migrations.AddUserLinksToArtistsAndAlbums do
  @moduledoc """
  Updates resources based on their most recent snapshots.

  This file was autogenerated with `mix ash_postgres.generate_migrations`
  """

  use Ecto.Migration

  def up do
    alter table(:artists) do
      add :created_by_id,
          references(:users,
            column: :id,
            name: "artists_created_by_id_fkey",
            type: :uuid,
            prefix: "public"
          )

      add :last_updated_by_id,
          references(:users,
            column: :id,
            name: "artists_last_updated_by_id_fkey",
            type: :uuid,
            prefix: "public"
          )
    end

    alter table(:albums) do
      add :created_by_id,
          references(:users,
            column: :id,
            name: "albums_created_by_id_fkey",
            type: :uuid,
            prefix: "public"
          )

      add :last_updated_by_id,
          references(:users,
            column: :id,
            name: "albums_last_updated_by_id_fkey",
            type: :uuid,
            prefix: "public"
          )
    end
  end

  def down do
    drop constraint(:albums, "albums_created_by_id_fkey")

    drop constraint(:albums, "albums_last_updated_by_id_fkey")

    alter table(:albums) do
      remove :last_updated_by_id
      remove :created_by_id
    end

    drop constraint(:artists, "artists_created_by_id_fkey")

    drop constraint(:artists, "artists_last_updated_by_id_fkey")

    alter table(:artists) do
      remove :last_updated_by_id
      remove :created_by_id
    end
  end
end
